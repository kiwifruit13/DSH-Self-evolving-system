 ...
 */
export const WRITE_METHODS: ReadonlySet<RpcMethod> = new Set<RpcMethod>([
  'init',
  'report_unknown',
  'planner_plan',
  'routing_split',
 ...
 * 无法推导也无法校验。现在显式落到契约里。
 */
export const TOOL_TO_METHOD: Readonly<Record<string, RpcMethod>> = {
...
export const TOOLED_METHODS: readonly RpcMethod[] = RPC_METHODS.filter(
  (m) => Object.values(TOOL_TO_METHOD).includes(m),
)

/** 工具名列表，顺序与 TOOL_TO_METHOD 的键顺序一致 */
export const TOOL_NAMES: readonly string[] = Object.keys(TOOL_TO_METHOD)

export function isWriteMethod(method: RpcMethod): boolean {
  return WRITE_METHODS.has(method)
}

/** 按方法名查工具名（反向映射，用于错误提示与测试断言） */
export function methodToTool(method: RpcMethod): string | null {
  for (const [tool, m] of Object.entries(TOOL_TO_METHOD)) {
    if (m === method) return tool
  }
  return null
}
